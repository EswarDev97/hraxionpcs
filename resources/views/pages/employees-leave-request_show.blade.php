    ...
    <div class="col-12">
      ...
      <div class="mb-3">
        ...
        <div class="row">
          <div class="col-sm-12 col-lg-6">
            <div class="form-group">
              <label for="remaining_leave">Remaining Leave Balance:</label>
              <input type="number" name="remaining_leave" class="form-control-plaintext" readonly
                value="{{ $employeeLeave->leaves_quota - $employeeLeave->used_leaves }}">
            </div>
          </div>
          <div class="col-sm-12 col-lg-6">
            <div class="form-group">
        ...
        <div class="row">
          ...
          <div class="col-sm-12 col-lg-6">
            <div class="form-group">
              ...
              <input type="text" name="checked_by" id="checked_by" class="form-control-plaintext" readonly value="{{ optional($employeeLeaveRequest->checkedBy)->name }}">
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-12">
            @if (auth()->user()->isAdmin() || auth()->user()->employee->id == $employeeLeaveRequest->employee->manager_id)
            <button type="button" class="btn btn-success mr-2 px-5" data-toggle="modal" data-target="#acceptModal">Accept</button>
            <button type="button" class="btn btn-danger mr-2 px-5" data-toggle="modal" data-target="#rejectModal">Reject</button>
            @endif
            @if (auth()->user()->employee->id == $employeeLeaveRequest->employee_id && $employeeLeaveRequest->status == 'WAITING_FOR_APPROVAL')
      ...
      <div class="modal-content">
        ...
        <form action="{{ route('employees-leave-request.update', ['employeeLeaveRequest' => $employeeLeaveRequest->id]) }}" method="POST">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              <label for="accept_comment">Comment:</label>
              <input type="text" name="comment" id="accept_comment" required placeholder="Enter comment" class="form-control @error('comment') is-invalid @enderror">
              @error('comment')
      ...
      <div class="modal-content">
        ...
        <form action="{{ route('employees-leave-request.destroy', ['employeeLeaveRequest' => $employeeLeaveRequest->id]) }}" method="POST">
          ...
          <div class="modal-body">
            <input type="hidden" name="checked_by" value="{{ auth()->user()->employee->id }}">
            <div class="form-group">
              <label for="reject_comment">Comment:</label>
              <input type="text" name="comment" id="reject_comment" required placeholder="Enter comment" class="form-control @error('comment') is-invalid @enderror">
              @error('comment')
              <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>
