    ...
    <div class="col-12 mb-3">
      <div class="bg-light text-dark card p-3 overflow-auto">
        <div class="d-flex justify-content-between">
          @if (collect($accesses)->where('menu_id', 10)->first() && collect($accesses)->where('menu_id', 10)->first()->status == 2)
        ...
        <table class="table table-light table-striped table-hover table-bordered text-center">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="w-25">
                @if ($user->employee)
                  <a href="{{ route('employees-data.show', ['employee' => $user->employee->id]) }}">{{ $user->name }}</a>
                @else
                  {{ $user->name }}
                @endif
              </td>
